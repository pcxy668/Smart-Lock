...
static char test_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "ESP_GATTS_LOCK";
...
#define CONFIG_EXAMPLE_CHAR_READ_DATA_LEN 24
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            if (!memcmp(param->write.value, "addpswd:", 8)) // 在nvs区存储新的密码
            {
                
                int32_t pswd =  (param->write.value[8] - '0') * 100000 + 
                                 (param->write.value[9] - '0') * 10000 + 
                                 (param->write.value[10] - '0') * 1000 + 
                                 (param->write.value[11] - '0') * 100 + 
                                 (param->write.value[12] - '0') * 10 + 
                                 (param->write.value[13] - '0');
                printf("addpswd:%ld\n", pswd);
                nvs_handle_t my_handle;
                nvs_open("storage", NVS_READWRITE, &my_handle);
                nvs_set_i32(my_handle, "password", pswd);
                nvs_commit(my_handle);
                nvs_close(my_handle);
            }
            else if(!memcmp(param->write.value, "openlock", 8)) // 蓝牙开锁命令
            {
                Int_Motor_OpenLock();
                printf("open lock\n");
            }
